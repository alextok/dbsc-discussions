...
Once a localkeyhelper is registered with the  Browser application, Browser applications can interact with the Local KeyHelper's ContentProvider by querying it. 
Below is an example of how a browser application can query specific LocalKeyhelper ContentProvider.
It can use the Packagemanager to get all the installed local keyhelper applications that provide a content provider and have registered.
...
for (PackageInfo packageInfo : installedPackages) {
    ...
    if (providers != null) {
        for (ProviderInfo provider : providers) {
            if (provider.authority.contains("localkeyhelper")) {
                // Add to the registered list of local keyhelpers
            }
        }
    }
}
```
##### Sample Code to Query the Local KeyHelper ContentProvider
    
```java
Uri uri = Uri.parse("content://com.contoso.localkeyhelper/sample_api_path");
Cursor cursor = getContentResolver().query(uri, null, null, null, null);
if (cursor != null) {
    try {
